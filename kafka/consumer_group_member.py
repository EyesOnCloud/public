...
# All three brokers in bootstrap list — client stays connected if one goes down
...
def on_assign(consumer, partitions):
    ...
    # Show which broker leads each assigned partition
    for p in partitions:
...
try:
    while not shutdown_flag:
        ...
        print(f"[{INSTANCE_NAME}] P{partition}:O{offset:4d} | "
              f"order={order_id} | "
